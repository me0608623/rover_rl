...
namespace onboardDetector{
    ...
    private:
        std::string ns_;
        std::string hint_;
        std::string visFrame_;

        // ROS
        std::shared_ptr<message_filters::Subscriber<ImageMsg>> depthSub_;
        std::shared_ptr<message_filters::Subscriber<PointCloud2Msg>> lidarCloudSub_;
        std::shared_ptr<message_filters::Subscriber<PoseStampedMsg>> poseSub_;
        typedef message_filters::sync_policies::ApproximateTime<ImageMsg, PoseStampedMsg> depthPoseSync;
        std::shared_ptr<message_filters::Synchronizer<depthPoseSync>> depthPoseSync_;
        typedef message_filters::sync_policies::ApproximateTime<PointCloud2Msg, PoseStampedMsg> lidarPoseSync;
        std::shared_ptr<message_filters::Synchronizer<lidarPoseSync>> lidarPoseSync_;
        std::shared_ptr<message_filters::Subscriber<OdometryMsg>> odomSub_;
        typedef message_filters::sync_policies::ApproximateTime<ImageMsg, OdometryMsg> depthOdomSync;
        std::shared_ptr<message_filters::Synchronizer<depthOdomSync>> depthOdomSync_;
        typedef message_filters::sync_policies::ApproximateTime<PointCloud2Msg, OdometryMsg> lidarOdomSync;
        std::shared_ptr<message_filters::Synchronizer<lidarOdomSync>> lidarOdomSync_;
        rclcpp::Subscription<ImageMsg>::SharedPtr colorImgSub_;
        ...
        rclcpp::Publisher<ImageMsg>::SharedPtr uvDepthMapPub_;
        rclcpp::Publisher<ImageMsg>::SharedPtr uDepthMapPub_;
        rclcpp::Publisher<ImageMsg>::SharedPtr uvBirdViewPub_;
        rclcpp::Publisher<ImageMsg>::SharedPtr detectedColorImgPub_;
        rclcpp::Publisher<MarkerArrayMsg>::SharedPtr uvBBoxesPub_;
        rclcpp::Publisher<MarkerArrayMsg>::SharedPtr dbBBoxesPub_;
        rclcpp::Publisher<MarkerArrayMsg>::SharedPtr visualBBoxesPub_;
        rclcpp::Publisher<MarkerArrayMsg>::SharedPtr lidarBBoxesPub_;
        rclcpp::Publisher<MarkerArrayMsg>::SharedPtr filteredBBoxesBeforeYoloPub_;
        ...
        rclcpp::Publisher<PointCloud2Msg>::SharedPtr dynamicPointsPub_;
        rclcpp::Publisher<PointCloud2Msg>::SharedPtr rawDynamicPointsPub_;
        rclcpp::Publisher<PointCloud2Msg>::SharedPtr downSamplePointsPub_;
        rclcpp::Publisher<PointCloud2Msg>::SharedPtr rawLidarPointsPub_;
        rclcpp::Publisher<MarkerArrayMsg>::SharedPtr historyTrajPub_;
        rclcpp::Publisher<MarkerArrayMsg>::SharedPtr velVisPub_;
        ...
        double dynamicMinDisp_ = 0.0;
        int dynamicDispWindow_ = 0;
    ...
    public:
        ...
        // helper for parameter loading (ROS2)
        template<typename T>
        T getParamHelper(const std::string& name, const T& defaultValue){
            T value = defaultValue;
            if (!this->has_parameter(name)){
                this->declare_parameter<T>(name, defaultValue);
            }
        ...
        // service
		void getDynamicObstacles(const std::shared_ptr<onboard_detector::srv::GetDynamicObstacles::Request> req,
								 std::shared_ptr<onboard_detector::srv::GetDynamicObstacles::Response> res);

        // callback
        void depthPoseCB(const ImageMsg::ConstSharedPtr& img, const PoseStampedMsg::ConstSharedPtr& pose);
        void depthOdomCB(const ImageMsg::ConstSharedPtr& img, const OdometryMsg::ConstSharedPtr& odom);
        void lidarPoseCB(const PointCloud2Msg::ConstSharedPtr& cloudMsg, const PoseStampedMsg::ConstSharedPtr& pose);
        void lidarOdomCB(const PointCloud2Msg::ConstSharedPtr& cloudMsg, const OdometryMsg::ConstSharedPtr& odom);
        void colorImgCB(const ImageMsg::ConstSharedPtr& img);
        ...
        void publishPoints(const std::vector<Eigen::Vector3d>& points, const rclcpp::Publisher<PointCloud2Msg>::SharedPtr& publisher);
        ...
        void getCameraPose(const PoseStampedMsg::ConstSharedPtr& pose, Eigen::Matrix4d& camPoseDepthMatrix, Eigen::Matrix4d& camPoseColorMatrix);
        void getCameraPose(const OdometryMsg::ConstSharedPtr& odom, Eigen::Matrix4d& camPoseDepthMatrix, Eigen::Matrix4d& camPoseColorMatrix);
        void getLidarPose(const PoseStampedMsg::ConstSharedPtr& pose, Eigen::Matrix4d& lidarPoseMatrix);
        void getLidarPose(const OdometryMsg::ConstSharedPtr& odom, Eigen::Matrix4d& lidarPoseMatrix);
    ...
    inline void dynamicDetector::getCameraPose(const PoseStampedMsg::ConstSharedPtr& pose, Eigen::Matrix4d& camPoseDepthMatrix, Eigen::Matrix4d& camPoseColorMatrix){
    ...
    inline void dynamicDetector::getCameraPose(const OdometryMsg::ConstSharedPtr& odom, Eigen::Matrix4d& camPoseDepthMatrix, Eigen::Matrix4d& camPoseColorMatrix){
        Eigen::Quaterniond quat;
    ...
    inline void dynamicDetector::getLidarPose(const PoseStampedMsg::ConstSharedPtr& pose, Eigen::Matrix4d& lidarPoseMatrix){
        Eigen::Quaterniond quat;
        quat = Eigen::Quaterniond(pose->pose.orientation.w, pose->pose.orientation.x, pose->pose.orientation.y, pose->pose.orientation.z);
        Eigen::Matrix3d rot = quat.toRotationMatrix();
    ...
    inline void dynamicDetector::getLidarPose(const OdometryMsg::ConstSharedPtr& odom, Eigen::Matrix4d& lidarPoseMatrix){
        Eigen::Quaterniond quat;
        quat = Eigen::Quaterniond(odom->pose.pose.orientation.w, odom->pose.pose.orientation.x, odom->pose.pose.orientation.y, odom->pose.pose.orientation.z);
        Eigen::Matrix3d rot = quat.toRotationMatrix();
