...
if [ "$n" -eq 0 ]; then
  ...
  ps -eo pid,cmd --no-headers | grep -E "deploy_full.launch|detector_node|policy_node|vo_safety|recovery_supervisor|orca_safety|ndt_localizer|voxel_grid_filter|world_to_map|yolov11|status_tui|lidar_preprocessor|mppi_planner|routing|costmap|mot_node|bev_play|map_loader|simple_map_publisher|yolo_venv" | grep -v grep | head
fi
